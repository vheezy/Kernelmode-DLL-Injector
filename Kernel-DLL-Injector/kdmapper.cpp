...
uint64_t kdmapper::MapDriver(HANDLE iqvw64e_device_handle)
{
	...
	{
		std::cout << xor ("[-] Image is not 64 bit") << std::endl;
		return 0;
	}
	...
	do
	{
		...
		{
			std::cout << xor ("[-] Failed to allocate remote image in kernel") << std::endl;
			break;
		}
		...
		//std::cout << xor ("[+] Skipped 0x") << std::hex << TotalVirtualHeaderSize << xor (" bytes of PE Header") << std::endl;
		...
		{
			std::cout << xor ("[-] Failed to resolve imports") << std::endl;
		...
		{
			std::cout << xor ("[-] Failed to call driver entry.") << std::endl;
			kernel_image_base = realBase;
			break;
		}
		...
		{
			//SetKdRedColor();
			std::cout << xor ("[-] Failed to init driver: Error 0x") << std::hex << std::setw(8) << std::setfill('0') << std::uppercase << status << std::nouppercase << std::dec << std::endl;
			kernel_image_base = realBase;
			break;
		}

		//std::cout << xor ("[+] Driver returned 0x") << std::hex << std::setw(8) << std::setfill('0') << std::uppercase << status << std::nouppercase << std::dec << std::endl;
...
bool kdmapper::ResolveImports(HANDLE iqvw64e_device_handle, portable_executable::vec_imports imports)
{
	for (const auto& current_import : imports)
	{
		...
		{
			std::cout << xor ("[-] Dependency ") << current_import.module_name << xor (" wasn't found") << std::endl;
			return false;
		}
